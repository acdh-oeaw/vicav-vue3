...
| `entry.entry.@lang`                                                                                             | `entryLang`, `metadata[]` item `entry language`      | Header metadata badge; `entryLang` itself is view-model-only.                              |
...
| form `@type`                               | `type`, `metadata[]` item `type`       | Metadata badge in lemma and inflected cards, except `type: lemma` is hidden in the Lemma row.           |
...
| sense `translationEquivalent_cit`, `translationEquivalent_cits[]` | `translations[]`                                    | Sense translation lines; `form.orth` is the main text and `gloss` renders in brackets next to it. |
