...
function parseRawItems(
	...
	return table.flatMap((item, itemIndex) => {
		...
		diagnostics.push(diagnostic("corpus", item, itemIndex, parsedCorpus.error));
		return [];
	});
}
...
function parseGeoItems(
	...
	return table.flatMap((item, itemIndex) => {
		...
		for (const candidate of candidates) {
			...
			const places = listPlace.flatMap((place, placeIndex) => {
				...
				if (!parsedPlace.success)
					diagnostics.push(
						diagnostic(
							"geographic-place",
							place,
							`${String(itemIndex)}:${String(placeIndex)}`,
							parsedPlace.error,
						),
					);
...
function extractMetadata(
	...
	diagnostics.push(diagnostic("metadata", item, item["@id"] ?? "unknown", parsedItem.error));
	return null;
}
 ...
 * process. Entries are shared by reference across concurrent requests and
...
const preparedResponses = new WeakMap<object, FrozenCacheEntry>();
...
/** Freeze plain data before sharing it across request stores. */
export function freezeCacheEntry(entry: CacheEntry): FrozenCacheEntry {
	deepFreeze(entry);
...
function diagnostic(
	source: ValidationDiagnostic["source"],
	item: unknown,
	index: string | number,
	error: z.ZodError,
): ValidationDiagnostic {
	const rawId = getRecordProperty(item, "@id");
	const itemId = typeof rawId === "string" ? rawId : undefined;
	const issues = error.issues.map(({ code, path, message }) => ({
		code,
		path: path.map((part) => (typeof part === "number" ? part : String(part))),
		message,
	}));
	return {
		id: JSON.stringify([source, index, itemId, issues]),
		source,
		index,
...
function buildCacheEntry(input: ProjectResponse): CacheEntry {
	...
	const parsed = parseRawItems(table, diagnostics);
	const simpleItems = buildSimpleItems(parsed, parseGeoItems(table, diagnostics), diagnostics);
	for (const [collectionIndex, collection] of (
		(input.projectConfig?._validationErrors ? [] : response.projectConfig?.staticData?.geo) ?? []
	).entries()) {
			...
			if (!result.success)
				diagnostics.push(
					diagnostic(
						"geojson",
						feature,
						`${String(collectionIndex)}:${String(index)}`,
						result.error,
					),
				);
			return result.success ? result.data : null;
		}) as typeof collection.features;
	}
	if (response.projectConfig) response.projectConfig._validationErrors = diagnostics;
...
export async function prepareProject(
	...
	const existing = preparedResponses.get(toRaw(input));
	if (existing) return existing;
	const etag = input.ETag;
	const key =
		etag && !options.authenticated
			? JSON.stringify([backendIdentity.replace(/\/$/, ""), etag])
			: null;
	if (!etag && !hasWarnedMissingEtag) {
		hasWarnedMissingEtag = true;
		console.warn(
			"[prepare-project] Project response carries no ETag; preparation is not memoized across requests.",
		);
	}
	if (!key) {
		const entry = freezeCacheEntry(buildCacheEntry(input));
		preparedResponses.set(entry.response, entry);
		return entry;
	}
	const cached = getCachedParsedCorpus(key);
	if (cached) return cached;
	let pending = inFlightParses.get(key);
	if (!pending) {
		pending = Promise.resolve().then(() => setCachedParsedCorpus(key, buildCacheEntry(input)));
		inFlightParses.set(key, pending);
	}
	try {
		...
		if (inFlightParses.get(key) === pending) inFlightParses.delete(key);
	}
}
